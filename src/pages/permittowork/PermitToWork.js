...
import { Print, Download, PlusCircle } from "../../icons/index";
...
export default function PermitToWork(params) {
  ...
  return (
    <>
          ...
          <ul className="relative h-80 overflow-y-scroll">
            <li className="flex flex-row py-4 px-6 items-center border gap-4 bg-white hover:border-gray-300 hover:shadow-sm duration-150 rounded-md mb-2">
              ...
              <div className="flex gap-2">
                <Button onClick={() => setIsOpenReview(!isOpenReview)} size="small" layout="outline">
                  Buka
                </Button>
              </div>
            </li>
      ...
      {isOpenReview && (<PermitToWorkReview isOpenReview={isOpenReview} setIsOpenReview={setIsOpenReview} />)}
    </>
  );
}
