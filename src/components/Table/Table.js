...
import { SortDownIcon, SortUpIcon, SortIcon } from "../../assets/Icon";
...
export function DisplayJam({ value }) {
  return <span>{moment(value).format("LT")}</span>;
}

export function DisplayBadgeStatus({value}){
    return <BadgeStatus status={value} />
}

export function DisplayProgressBarSmall({value}){
    return <ProgressbarSmall progress={value} />
}

export function DisplayNomorRekam({ value }) {
  const sbr1 = value.substring(1, 3);
  const sbr2 = value.substring(3, 5);
  const sbr3 = value.substring(5, 7);

  const result = [sbr1, sbr2, sbr3].join(" ");
  return result;
}
                      ...
                      <tr {...row.getRowProps()}>
                        {row.cells.map((cell) => {
                          return (
                            <td
                              {...cell.getCellProps()}
                              className="text-sm px-4 py-2 whitespace-nowrap truncate"
                            >
                              {cell.render("Cell")}
                            </td>
                          );
        ...
        <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
          <div className="flex gap-x-2 items-center">
            <span className="text-sm text-gray-700">
              Halaman <span className="font-medium">{state.pageIndex + 1}</span>{" "}
              dari <span className="font-medium">{pageOptions.length}</span>
            </span>
            <label>
